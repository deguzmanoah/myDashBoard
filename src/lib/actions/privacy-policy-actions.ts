...
export interface PrivacyPolicyFormState {
  success: boolean;
...
// Schema for privacy policy validation - ID is no longer required
...
export async function updatePrivacyPolicy(
  prevState: PrivacyPolicyFormState,
  formData: FormData
  ...
  try {
    ...
    const validatedData = privacyPolicySchema.parse(rawData);
    ...
    // Make authenticated API request to update privacy policy
    ...
    revalidatePath('/dashboard/app-cms/privacy-policy');
...
export async function getPrivacyPolicy(usePublicAPI: boolean = false): Promise<{
  success: boolean;
  data?: {
    id: string;
    title: string;
  ...
  try {
    ...
    if (usePublicAPI && !data.is_published) {
      return { success: false, error: 'Privacy policy is not published' };
    }
