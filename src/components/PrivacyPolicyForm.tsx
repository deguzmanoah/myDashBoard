...
} from '@/lib/actions/privacy-policy-actions';
...
interface PrivacyPolicyFormProps {
  initialData: PrivacyPolicyFormData; // Required - data comes from server
  onCancel?: () => void;
...
export default function PrivacyPolicyForm({
  ...
  const [formData, setFormData] = useState<PrivacyPolicyFormData>(initialData);

  const [clientErrors, setClientErrors] = useState<Record<string, string>>({});
  const [isSubmitting, setIsSubmitting] = useState(false);

  // Use server action with useActionState - always updating existing data
  const [formState, formAction] = useActionState<PrivacyPolicyFormState, FormData>(
    updatePrivacyPolicy,
  ...
  useEffect(() => {
    ...
    if (formState.success) {
      ...
      router.push('/dashboard/app-cms/privacy-policy');
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        ...
        description="Update privacy policy content for the mobile application"
      />
